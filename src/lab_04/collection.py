...
class ServerCollection:
    ...
    # новые методы для ЛР-4, фильтрация по интерфейсу
    
    def get_printable(self):
        """Вернуть коллекцию объектов, реализующих интерфейс Printable"""
        
        result = ServerCollection()
        for server in self._items:
            if isinstance(server, Printable):
                result.add(server)
        return result
    
    def get_comparable(self):
        """Вернуть коллекцию объектов, реализующих интерфейс Comparable"""
        
        result = ServerCollection()
        for server in self._items:
            if isinstance(server, Comparable):
                result.add(server)
        return result